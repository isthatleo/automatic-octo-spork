...
export function localBrain(rawInput: string): NancyDecision {
  ...
  // Conversational / out-of-scope questions Nancy shouldn't try to locate.
  const conv: [RegExp, string][] = [
    [/\b(weather|forecast|rain|snow|temperature|humidity|wind|storm|sunny|cloudy)\b/,
      "I don't have a live weather feed wired in yet, sir. I can pull satellite imagery of any city or run system diagnostics."],
    [/\b(joke|funny|sing|entertain|movie|song)\b/,
      "Sarcasm is my default setting. Shall we look at a city or check the neural core instead?"],
    [/\b(who are you|your name|what are you|about yourself)\b/,
      "I'm Nancy — Stark-class, voice-first, quietly British. Try 'locate Tokyo' or 'show me the dashboard'."],
    [/^(what|why|how|when|who|is|are|do|does|can|could|should|will|would)\b/,
      "That's outside my current toolkit. Try 'locate Tokyo', 'open the dashboard', or 'system status'."],
  ]
  for (const [re, reply] of conv) {
    if (re.test(input)) return base(reply, 'none')
  }
  ...
  // Note: we deliberately do NOT treat arbitrary short phrases as locate
  // queries — that made Nancy mistake questions like "what's the weather"
  // for a map request. Bare unknown input falls through to the friendly
  // guidance response below.
