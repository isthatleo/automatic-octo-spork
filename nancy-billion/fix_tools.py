...
def fix_tools_file():
    try:
        # Read the original file as UTF-16
        with open("C:\\Users\\leona.DESKTOP-10QNDAN\\Desktop\\automatic-octo-spork\\nancy-billion\\backend\\tools.py", "r", encoding="utf-16") as f:
            content = f.read()
        
        # Find where to insert the learning tools (before the logger.info line)
        # Look for the last occurrence of the security_hardening tool registration end
        # Actually, let's find the line with "logger.info(f\"Created {len(tools)} tools\")"
        lines = content.split('\n')
        insert_index = -1
        for i, line in enumerate(lines):
            if 'logger.info(f"Created {len(tools)} tools")' in line:
                insert_index = i
                break
        ...
        # Define the learning tool functions
        learning_functions = '''
    # =============================================================================
    ...
    def hypothesis_generation_execute(domain: str = "", observation_data: str = "", num_hypotheses: int = 3) -> str:
        ...
        if not observation_data:
            observation_data = "Global temperatures have risen 1.2°C since pre-industrial times, with accelerated warming in Arctic regions."
        ...
        if "team" in experience_description.lower() or "conflict" in experience_description.lower() or "leadership" in experience_description.lower():
            ...
            wisdom_principles = [
                ...
                },
                {
                    ...
                    "explanation": "Understanding the underlying principles allows application to novel situations that surface-level memorization cannot handle",
...
'''
        ...
        # Define the learning tool registrations
...
'''
        ...
        # Insert the learning tools before the logger.info line
        lines_before = lines[:insert_index]
        lines_after = lines[insert_index:]
        
        # Combine everything
        new_content = '\n'.join(lines_before) + learning_functions + learning_registrations + '\n'.join(lines_after)
        
        # Write back as UTF-8 (standard encoding)
        with open("C:\\Users\\leona.DESKTOP-10QNDAN\\Desktop\\automatic-octo-spork\\nancy-billion\\backend\\tools.py", "w", encoding="utf-8") as f:
            f.write(new_content)
            
        print("Successfully updated tools.py with learning tools")
        return True
        
    except Exception as e:
        print(f"Error fixing tools.py: {e}")
        import traceback
        traceback.print_exc()
        return False
