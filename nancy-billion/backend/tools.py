"""
Tool definitions for Nancy/Billion's Fury-based agents.

RECOVERY NOTE (2026-07-12): this file was found with a corrupted header —
null bytes, mixed \\r\\r\\n/\\r\\n line endings, and double-escaped quotes — and
was, more seriously, missing its `logger`, `add_tool`, and `get_tools`
definitions entirely (only ~630 lines of `add_tool(...)` call sites remained).
Since `main_new.py` does `from tools import get_tools` as an unconditional
top-level import, this meant the backend could not start at all. Reconstructed
below from first principles: `add_tool`'s call sites use the exact 5-positional-arg
shape of `fury.types.create_tool(id, description, execute, input_schema,
output_schema) -> Tool` (verified against fury-main/src/fury/types.py), so
`add_tool` is a thin wrapper appending a `Tool` to a module-level list, and
`get_tools()` returns that list.
"""
...
def add_tool(id, description, execute, input_schema, output_schema):
    """Register a tool, converting it to a fury Tool via create_tool()."""
    _TOOLS.append(create_tool(id, description, execute, input_schema, output_schema))


def get_tools() -> list:
    """Return all registered tools (consumed by main_new.py, agent_executor.py, llm.py)."""
    return _TOOLS
